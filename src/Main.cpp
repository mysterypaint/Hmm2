...
double pulse(int t, double freq, double amp, double pulse)
{
	return amp * sgn((fmod(freq * t / SAMPLERATE, 1.0) <= pulse));
}
...
/*
		...
		//s16 sample = 0.3 * 0x7FFF * sin(frequency/2 * (2 * M_PI) * (offset + i) / SAMPLERATE);
		
		//double square(int t, double freq, double amp, double pulse)
		
		s16 ch1 = 0x7FFF * pulse(offset+i, frequency, 0.3, 0.25);
		s16 ch2 = 0x7FFF * pulse(offset+i, frequency*2, 0.3, 0.5);
		s16 ch3 = 0.3 * 0x7FFF * sin(frequency/2 * (2 * M_PI) * (offset + i) / SAMPLERATE);
		
		s16 wave_out = ch1 + ch2 + ch3;
		// Stereo samples are interleaved: left and right channels.
		dest[i] = (wave_out << 16) | (wave_out & 0xffff);
	}
